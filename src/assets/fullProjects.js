const frontData = [
    {
        key: 6,
        name: 'Travel-Mate',
        gitLink: 'https://github.com/rhemlock7/TeamSync-KanBan?tab=readme-ov-file',
        smallDesc: "Task manager designed to help developers organize and manage their tasks effectively using the Kanban methodology ",
        icons: ['react.svg', 'vite.svg', 'tailwind.svg', 'redux.svg', 'css.svg', 'mongo.svg', 'mongoose.png', 'js.svg', 'node.svg', 'github.svg'],
        bgImage: 'teamsync.jpg',
        role: 'Created Queries, Mutations and Modules for Back-end structure and added it to the Front-end',
        features: ['Log In, Sign Up', 'User authentication', `User's ability to create new Projects and add to it ToDoes`, `Ability to add multiple users to one project`],
        liveLink: 'https://teamsync-client-project-3.netlify.app/'
    },
    {
        key: 1,
        name: 'Travel-Mate',
        gitLink: 'https://github.com/MykhailoZakh/Travel-Mate',
        smallDesc: "Front-End with Back-End Data Base travel app ",
        icons: ['css.svg', 'handlebars.png', 'js.svg', 'node.svg', 'sql.svg', 'sequelize.svg', 'github.svg'],
        bgImage: 'travel.png',
        role: 'Created Data structure and added it to the Front-end',
        features: ['Log In, Sign Up', 'Session Logged-in user  authorization', `User's capability to add new posts or events to the website`, `User's Profile with user's icon`],
        liveLink: ''
    },

    {
        key: 3,
        name: 'Dev-Blog',
        gitLink: 'https://github.com/MykhailoZakh/Dev-Blog',
        smallDesc: "Front end app to add posts and keep it inside Data Base",
        icons: ['css.svg', 'handlebars.png', 'js.svg', 'node.svg', 'sql.svg', 'sequelize.svg', 'github.svg'],
        bgImage: 'devb.png',
        role: "Solo developer",
        features: ['User authorization to be able to add posts or comments', "User's ability to add new posts and comments to posts"],
        liveLink: ''
    },
    {
        key: 2,
        name: 'Your-Nutrition-Radar',
        gitLink: 'https://github.com/MykhailoZakh/Your-Nutrition-Radar',
        smallDesc: "Front end app to search recepies",
        icons: ['css.svg', 'html.svg', 'js.svg', 'github.svg'],
        bgImage: 'nutrition.png',
        role: 'Added API and functionalities to application',
        features: ['Implementing APis', 'Ability to search recepies', 'Ability to add different tags to search', 'Power to store favorite recepies in local storage'],
        liveLink: ''
    },
    {
        key: 4,
        name: 'My-Note-Reminder',
        gitLink: 'https://github.com/MykhailoZakh/My-Note-Reminder',
        smallDesc: "Front-End app to add notest and save it inside web Local Storage ",
        icons: ['css.svg', 'html.svg', 'js.svg', 'github.svg'],
        bgImage: 'note.png',
        role: "Solo developer",
        features: ['Adding and deliting inforamtion from local storage'],
        liveLink: ''
    },
    // {
    //     key: 5,
    //     name: 'Coding-Quiz-Game',
    //     gitLink: 'https://github.com/MykhailoZakh/Coding-Quiz-Game',
    //     smallDesc: "Front-End app to play code game and save score inside web Local Storage ",
    //     icons: ['css.svg', 'html.svg', 'js.svg', 'github.svg'],
    //     bgImage: 'quiz.png',
    //     role: "Solo developer",
    //     features: [''],
    //     liveLink: ''
    // }
]

export default frontData